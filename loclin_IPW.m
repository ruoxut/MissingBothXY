function [ t,f ] = loclin_IPW( a,b,X,Y,RX,RY,U,alpha )
% The inverse probability weighting local linear estimator where both X and
% Y are subject to missingness using the linear g_{r^x,r^y}'s.
% Input:
...
hCV3 = @(h) CVloss3_m( h,X,Y,U,alpha,a,b);
h2 = fminbnd(hCV3,(b-a).*length(X)^(-0.5),(b-a)./2,opt);
fCV3 = zeros(length(X),1);% \hat{m}^{(2)}

for i = 1:length(X)
    if X(i) >= a && X(i) <=b
    a0 = normpdf((X-X(i))./h2)./h2./pi_11(X,Y,U,alpha);
    a1 = (X-X(i)).*a0;
    a2 = (X-X(i)).*a1;
    a3 = (X-X(i)).*a2;
    a4 = (X-X(i)).*a3;
...
Theta_2 = sum(fCV3.^2./pi_11(X,Y,U,alpha).*(a<=X&X<=b))/length(RX);

hCV1 = @(h) CVloss1_m( h,X,Y,Y,U,alpha,a,b);
h3 = fminbnd(hCV1,(b-a).*length(X)^(-0.5),(b-a)./2,opt);
fCV1 = zeros(length(X),1);% \hat{m}_{lin}

for i = 1:length(X)
    if X(i) >= a && X(i) <=b
    a0 = normpdf((X-X(i))./h3)./h3./pi_11(X,Y,U,alpha);
    a1 = (X-X(i)).*a0;
    a2 = (X-X(i)).*a1;
...
sigma2 = (Y-fCV1).^2./pi_11(X,Y,U,alpha);
hCVS1 = @(h) CVloss1_m( h,X,Y,sigma2,U,alpha,a,b);
h4 = fminbnd(hCVS1,(b-a).*length(X)^(-0.5),(b-a)./2,opt);
sigma_CV = zeros(length(t),1);

for i = 1:length(t)
    a0 = normpdf((X-t(i))./h4)./h4./pi_11(X,Y,U,alpha);
    a1 = (X-t(i)).*a0;
    a2 = (X-t(i)).*a1;
...
for i = 1:length(t)
    a0 = normpdf((X-t(i))./h_PI)./h_PI./pi_11(X,Y,U,alpha);
    a1 = (X-t(i)).*a0;
    a2 = (X-t(i)).*a1;
