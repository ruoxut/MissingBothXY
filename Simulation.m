...
for k = 1:2
    ...
    for i = 1:n_r
        ...
        [alpha_CMLE(1:1+size(U,2))] = LogCMLE(U_sub,RU_sub);
        ...
        [alpha_CMLE(2+size(U,2):3+2*size(U,2))] = LogCMLE(X_subx,RY_subx);
        ...
        [alpha_CMLE(4+2*size(U,2):5+3*size(U,2))] = LogCMLE(Y_suby,RX_suby);
        ...
        [ t,S(:,i,3) ] = loclin_IPW( a,b,X,Y,RX,RY,U,alpha_MLE );%m_MLE
    ...
    % Store results.
    fname = sprintf('BM210415_variablen%d',n);
    save(fname,'ISE','S');
